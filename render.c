...
void draw_triangle(SDL_Renderer* renderer, const Triangle* tri, const double light_factor) {
    ...
    // Check for counter-clockwise winding (backface culling)
    if (abc > 0) {
    ...
    // Find topmost vertex to use as initial seed
    ...
    // Use the x-coordinate of the topmost vertex as initial seed
    int seed_x = (int)matrix_get(tri->vertices[top_vertex_idx], 0, 0);
    
    // Scan each horizontal line of the triangle
    for (int y = min_y; y <= max_y; y++) {
        ...
        if (!in_tri) {
            ...
            if (!in_tri) continue;  // No points in triangle for this scanline
        }
        
        // Now x is a point known to be inside the triangle
        // Draw this point
        const double bc_a = bcp / abc;
        const double bc_b = cap / abc;
        const double bc_c = abp / abc;
        
        // Calculate colors using barycentric coordinates
